...
- Seleção das bandas de visada vertical e visada horizontal para o cálculo da amplitude
...
- Cálculo e extração de amplitude (distância euclidiana entre **VV** e **VH**)
...
- É aplicada a padronização das imagens enquanto em formato de *dataset*
...
f.add_subplot(1,3, 3).set_title("Band 20")
pkg.show_tif('assets/scaled.tif', 20)
```

## Análise

- Com uma imagem `.tif` representando a série temporal de imagens, a técnica de envelope espectral pode ser aplicada pixel à pixel por meio da biblioteca `RPy2`, extraindo a função `specenv` do pacote `{astsa}` para *R*
...
- Os coeficientes são escolhidos com base na frequência mais expressiva para o cálculo de uma transformação otimizada
...
::: {.column width="40%"}
- Sendo $\beta$ o autovetor equivalente para a frequência ótima $w$, têm-se que $\hat{\beta}(w) = (\beta_1, \beta_2, \beta_3)$.
...
- O vetor de mapeamento utilizado ocasionada em matrizes singulares, dificuldade o tratamento de diversas áreas da série de imagens
